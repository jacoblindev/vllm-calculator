...
// ================================
// EXPORTS - Re-export functions from imported modules and local functions
// ================================

// Re-export from memory modules for backward compatibility
export {
  calculateVRAMBreakdown,
  calculateModelWeightsMemoryFromSize,
  calculateOptimalSwapSpace,
...
export {
  calculateKVCacheMemory,
  estimateKVCacheForModel,
  calculateOptimalKVCacheSize,
  getKVCacheRecommendations
} from './memory/kvCache.js'

export {
  calculateActivationMemory,
  estimateActivationMemoryForModel,
  calculateOptimalActivationConfig,
  getActivationMemoryRecommendations
} from './memory/activations.js'

// Re-export from quantization module  
export {
  calculateModelWeightsMemory,
  calculateQuantizationFactor,
  getSupportedQuantizationFormats,
...
// Re-export from throughput optimization module
export {
  THROUGHPUT_OPTIMIZATION_CONFIGS,
  calculateOptimalBatchSize,
  calculateMemoryAllocationStrategy,
...
// Export local balanced optimization functions
export {
...
// Export local utility functions
export function calculateVRAMUsage(
  ...
  try {
    ...
    const weightsMemory = calculateModelWeightsMemory(modelSize, quantization)
    
    // Calculate KV cache memory
    const kvCacheMemory = calculateKVCacheMemory(batchSize, sequenceLength, numLayers, hiddenSize, numAttentionHeads)
    
    // Calculate activation memory
    const activationMemory = calculateActivationMemory(batchSize, sequenceLength, hiddenSize, numLayers)
    
    // Total VRAM usage
    const totalVRAM = weightsMemory.totalMemory + kvCacheMemory + activationMemory
    
    return {
      totalVRAM,
      breakdown: {
        weights: weightsMemory.totalMemory,
        kvCache: kvCacheMemory,
        activations: activationMemory
      },
      quantization: weightsMemory.quantization
    }
  } catch (error) {
    throw new Error(`VRAM calculation failed: ${error.message}`)
  }
}

export function canRunOnGPU(gpuVRAM, requiredVRAM) {
  if (typeof gpuVRAM !== 'number' || typeof requiredVRAM !== 'number') {
    throw new Error('GPU VRAM and required VRAM must be numbers')
  }
  
  if (gpuVRAM <= 0 || requiredVRAM <= 0) {
    throw new Error('GPU VRAM and required VRAM must be positive numbers')
  }
  
  // Reserve 20% of GPU memory for system overhead
  const usableVRAM = gpuVRAM * 0.8
  
  return {
    canRun: usableVRAM >= requiredVRAM,
    usableVRAM,
    requiredVRAM,
    overhead: gpuVRAM - usableVRAM,
    efficiency: requiredVRAM / usableVRAM
  }
}
